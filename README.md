...
Збереження draft нотатки у процесі її створення. Доповнено функціонал створення нотатки так, щоб під час заповнення полів форми в компоненті NoteForm.tsx автоматично зберігалася «чернетка» (draft) за допомогою Zustand. Це дозволить користувачу не втрачати незбережені зміни та при повторному відкритті форми побачити останній збережений стан. </br>
